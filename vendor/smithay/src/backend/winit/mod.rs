    ...
    crate::backend::SwapBuffersError: From<R::Error>,
{
    ...
    // The event loop is integrated into calloop as an event source. Poll mode
    // keeps that source ready even when no new frame is presented, which can
    // spin the nested compositor at a full CPU core. Let calloop sleep until
    // winit or another registered source has work.
